...
1. `build_backend(provider, model, ...)` takes plain keyword arguments directly, and 
2. `build_backend_from_resolved(resolved)` takes an `oa_configurator.ResolvedModel` (from `Resolver(stack).resolve_model(name)`) and does the field mapping for you. 
